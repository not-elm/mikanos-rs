...
impl KernelLoadable for KernelElfLoader {
    fn load(&mut self, kernel_file: &mut RegularFile, system_table: &mut SystemTable<Boot>) -> crate::error::Result<*mut u8> {
        ...
        let buff: &mut [u8] = unsafe { core::slice::from_raw_parts_mut(entry_point, kernel_file_size as usize) };
        read_kernel_buff(kernel_file, buff);

        println!("kernel buff = {:?}", buff);
...
fn allocate_pages(system_table: &mut SystemTable<Boot>,
    ...
    system_table
        .boot_services()
        .allocate_pages(AllocateType::Address(PhysicalAddress::from(kernel_base_addr)), MemoryType::LOADER_DATA, kernel_page_size)
}
